...
def test_lmm_kron2sum():
    ...
    lmm.cov.Cn.L0 = random.randn(3)
    K = lmm.cov.value()
    assert_allclose(lmm.lml(), st.multivariate_normal(m, K).logpdf(y))

    lmm.cov.Cn.L1 = random.randn(3)
    K = lmm.cov.value()
    assert_allclose(lmm.lml(), st.multivariate_normal(m, K).logpdf(y))
...
def test_lmm_kron2sum_gradient():
    ...
    def grad(x):
        ...
        lmm.mean.B = x[9:].reshape((2, 3), order="F")
        D = lmm.lml_gradient()
        return concatenate((D["Cr.Lu"], D["Cn.L0"], D["Cn.L1"], D["M.vecB"]))
...
def test_lmm_kron2sum_fit_Cn_well_cond_Cr_fullrank():
    random = RandomState(0)
    Y = random.randn(5, 2)
    A = random.randn(2, 2)
    A = A @ A.T
...
def test_lmm_kron2sum_fit_Cn_well_cond_redutant_G():
    random = RandomState(0)
    Y = random.randn(5, 2)
    A = random.randn(2, 2)
    A = A @ A.T
...
def test_lmm_kron2sum_fit_Cn_well_cond_redutant_F():
    random = RandomState(0)
    Y = random.randn(5, 2)
    A = random.randn(2, 2)
    A = A @ A.T
...
def test_lmm_kron2sum_fit_Cn_well_cond_redundant_Y():
    random = RandomState(0)
    Y = random.randn(5, 2)
